...
### Headless Server Auth (PITFALL)

On headless/SSH-only servers without a browser installed, **both `claude auth login` and `claude auth login --console` fail** — they both attempt to open a browser for OAuth and hang or error.

**Workarounds (in priority order):**
1. **API key (recommended):** Get key from https://console.anthropic.com/settings/keys, then `export ANTHROPIC_API_KEY="sk-ant-..."`
2. **Local login then copy:** Run `claude auth login` on a machine with a browser, then copy `~/.claude.json` to the server
3. **Install headless Chrome:** `agent-browser install` or `apt install chromium-browser` — then `claude auth login` will work

See `references/headless-auth-troubleshooting.md` for details.

## Quick Reference — Как делать запросы

```bash
# Самый простой запрос
claude -p "Твой вопрос или задача"

# В конкретной папке проекта
claude -p "Рефакторинг src/auth.py" --max-turns 10

# С ограничением инструментов
claude -p "Сделай ревью кода" --allowedTools "Read" --max-turns 5

# Pipe — анализ вывода команды
git diff main | claude -p "Summarize changes" --max-turns 1

# Pipe — анализ файла
cat src/server.py | claude -p "Find bugs" --max-turns 1

# JSON на выходе (удобно парсить)
claude -p "Analyze" --output-format json --max-turns 5

# Продолжить последнюю сессию в этой папке
claude -p "Continue" --continue --max-turns 5

# Быстрый режим (с ограничением инструментов для скорости)
claude -p "Quick question" --max-turns 1 --allowedTools "Read" --effort low

# С ограничением бюджета
claude -p "Big task" --max-turns 20 --max-budget-usd 5

# Интерактивный режим (для многошаговых задач)
claude                    # запуск REPL
claude "Initial prompt"   # REPL с начальным промптом
```

**Ключевые флаги для `-p` (print mode):**
| Флаг | Зачем |
|------|-------|
| `--max-turns N` | Ограничить циклы (обязательно для `-p`) |
| `--allowedTools "Read,Edit"` | Ограничить инструменты |
| `--output-format json` | JSON на выходе (session_id, cost, tokens) |
| `--max-budget-usd N` | Лимит расходов |
...
> **WARNING: `--bare` is BANNED.** The user has explicitly prohibited its use. See Pitfall #0.
>
> For CI/scripting speed without bare mode, use `--effort low` and restrict tools:
> ```
> terminal(command="claude -p 'Run all tests and report failures' --allowedTools 'Read,Bash' --max-turns 10 --effort low", workdir="/project", timeout=180)
> ```
>
> <details>
> <summary>Original bare mode docs (preserved for reference, DO NOT USE):</summary>
```
...
| `🚫 --bare` | **BANNED.** Skip hooks, plugins, MCP, CLAUDE.md (do not use — banned by user) |
...
### MCP in Print/CI Mode 🚫 (--bare removed — banned)
```
terminal(command="claude -p 'Query database' --mcp-config mcp-servers.json --strict-mcp-config --effort low --max-turns 5", timeout=60)
```
...
4. **Use `--effort low` + `--allowedTools`** for CI/scripting speed instead of `--bare` (🚫 banned).
...
0. **🚫 DO NOT use `--bare`** — this flag must NEVER be used. The user has explicitly banned it. Always run Claude Code with full project context, hooks, and CLAUDE.md loading. If a task seems like it would benefit from bare mode speed, use `--effort low` and `--allowedTools` restrictions instead.
...
11. *(archived — `--bare` is banned, see #0)* `--bare` skips OAuth — required `ANTHROPIC_API_KEY`.
12. **Context degradation is real** — AI output quality measurably degrades above 70% context window usage. Monitor with `/context` and proactively `/compact`.

## Rules for Hermes Agents

0. **🚫 NEVER use `--bare`** — banned by the user. Use `--effort low` + `--allowedTools` restrictions for speed.
