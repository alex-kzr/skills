...
**Deeper debugging when config looks correct but STT still fails:**

The gateway may report "no STT provider is configured" even when `faster-whisper` is installed and config is valid. This can happen after a Docker container restart if the gateway process didn't pick up the package. Debug by testing the transcription pipeline directly:

```bash
# 1. Verify the package is importable
python3 -c "from faster_whisper import WhisperModel; print('OK')"

# 2. Find cached voice files (gateway saves .ogg to disk before transcribing)
ls /hermes/cache/audio/*.ogg 2>/dev/null

# 3. Test transcription end-to-end on a cached file
python3 -c "
from tools.transcription_tools import transcribe_audio
result = transcribe_audio('/hermes/cache/audio/audio_XYZ.ogg')
print(result)
"
```

If step 3 returns `{'success': True, 'transcript': '...', 'provider': 'local'}` then STT is functional — the issue is the gateway not invoking it. A gateway restart (`/restart` in gateway, or docker restart) should fix it.

**Model download timing pitfall:** `faster-whisper` downloads the Whisper model from HuggingFace Hub on first transcription call, NOT at install time. If a voice message arrives before the model finishes downloading (~30s for `base`), the gateway gets a transcription error and sends "no STT provider is configured" to the agent. After the first successful download, the model is cached at `$HF_HOME/hub/models--Systran--faster-whisper-<size>/` (typically `/hermes/home/.cache/huggingface/hub/`). To pre-warm the cache after install, run:

```bash
python3 -c "from faster_whisper import WhisperModel; WhisperModel('base', device='cpu', compute_type='int8')"
```

**Gateway internals for deep debugging:**
- Voice pipeline: Telegram adapter caches `.ogg` → `gateway/run.py:~11900` `_enrich_message_with_transcription()` → `tools/transcription_tools.py:transcribe_audio()` → faster-whisper
- Audio cache: `/hermes/cache/audio/` (files named `audio_<hash>.ogg`)
- Gateway config object: `gateway/config.py:GatewayConfig.stt_enabled` reads from `stt.enabled` in config.yaml
- The `_HAS_FASTER_WHISPER` flag in `transcription_tools.py` is set at import time via `_safe_find_spec()` — if the package isn't on `sys.path` when the gateway starts, it stays `False` until restart
- Gateway logs: check `~/.hermes/logs/gateway.log` for "Cached user voice" (confirms download) and any transcription error lines

**Docker-specific notes:**
- Hermes running inside Docker may have `faster-whisper` installed in `/hermes/home/.local/lib/python3.12/site-packages/` (user site-packages) while the Hermes package lives in `/usr/local/lib/python3.12/site-packages/`. Both are on `sys.path`, so imports work — but verify with `python3 -c "import faster_whisper; print(faster_whisper.__file__)"`.
- Docker CLI is typically NOT available inside the container. Restart must be done externally: `docker restart <container>`.
- Audio cache lives at `/hermes/cache/audio/` — useful for replay-testing without sending new voice messages.

**Container self-recreate (modifying Hermes config from inside Hermes):**
- `docker restart` does NOT reload environment variables — the container must be destroyed and recreated.
- `docker compose` / `docker-compose` are often unavailable inside the container.
- Manual recreate via `docker run` requires preserving ALL labels (especially Traefik), volumes, network, restart policy, and group membership. Use Python `subprocess.run()` with list args to avoid bash escaping issues with backticks in Traefik labels.
- Full step-by-step recipe: `references/docker-container-self-recreate.md`.

Full pipeline architecture, failure mode catalog, and manual testing commands: `references/stt-debugging-pipeline.md`.
