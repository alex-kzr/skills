...
def _add_runtime_site_packages(start: Path) -> None:
    """Best-effort: this script may also run from a ~/.hermes copy outside
    the agents repo, so missing runtime/ is not fatal — fall back silently
    to whatever is already importable."""
    for parent in [start, *start.parents]:
        if (parent / "skills").exists() and (parent / "runtime").exists():
            venv_dir = parent / "runtime" / "python" / "base" / ".venv"
            if os.name == "nt":
...
try:
    _add_runtime_site_packages(Path(__file__).resolve())
except NameError:
    pass  # __file__ not defined when loaded via exec()
...
def race_models(query, tier="standard", api_key=None, system_prompt=None,
    ...
    """
    if OpenAI is None:
        raise ImportError("openai package required. Prepare agents/runtime/python/base/.venv (see runtime/README.md).")
