...
function findAgentsRoot(startDir) {
  let current = path.resolve(startDir);

  while (true) {
    const runtimePath = path.join(current, 'runtime');
    const skillsPath = path.join(current, 'skills');

    if (fs.existsSync(runtimePath) && fs.existsSync(skillsPath)) {
      return current;
    }
    ...
    if (parent === current) {
      throw new Error('Cannot find agents root with both skills/ and runtime/');
    }
...
function addRuntimeNodeModules() {
  const agentsRoot = findAgentsRoot(__dirname);
  const nodeModulesPath = path.join(agentsRoot, 'runtime', 'node', 'node_modules');

  if (!fs.existsSync(nodeModulesPath)) {
    throw new Error(
      `Node.js runtime is not prepared. Expected node_modules at: ${nodeModulesPath}`
    );
  }
