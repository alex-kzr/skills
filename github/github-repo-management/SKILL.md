...
description: "Clone/create/fork repos; manage remotes, releases, authentication (HTTPS tokens, SSH keys, gh CLI), and issues (create, triage, label, assign). Formerly split into github-auth + github-issues + github-repo-management."
version: 1.1.0
author: Hermes Agent
license: MIT
...
```bash
pip install pygount  # or: uv pip install pygount
```
...
### Filter by Language / Output Formats
...
2. **Markdown shows 0 code lines** — pygount classifies all Markdown as comments, not code.
...
Before working with GitHub repos, ensure authentication is configured.
...
2. If `gh` installed but not authenticated → run `gh auth login`.
3. If `gh` not installed → use git-only method below.
...
3. Test: `git ls-remote https://github.com/<user>/<repo>.git` (username + token as password).
4. Alternative cache: `git config --global credential.helper 'cache --timeout=28800'`
...
### gh CLI Setup
```bash
gh auth login  # Interactive: choose HTTPS, login with browser or token
...
### Creating Issues
```bash
gh issue create --title "Bug: description" --body "Details..." --label "bug" --assignee @me
```
...
### Assigning and Triage
```bash
gh issue edit 42 --add-assignee "username" --add-label "in-progress"
gh issue close 42 --comment "Fixed in #43"
```

### REST API Fallback (curl)
```bash
GITHUB_TOKEN=$(grep "github.com" ~/.git-credentials 2>/dev/null | head -1 | sed 's|https://[^:]*:\([^@]*\)@.*|\1|')
curl -s -H "Authorization: token $GITHUB_TOKEN" \
  "https://api.github.com/repos/$OWNER/$REPO/issues?state=open" | python3 -c "
import sys, json
for i in json.load(sys.stdin):
    if 'pull_request' not in i:
        print(f\"#{i['number']:5} {i['title']}\")
"
```
