...
def _add_ml_runtime_site_packages() -> None:
    agents_root = _find_agents_root(Path(__file__))
    venv_dir = agents_root / "runtime" / "python" / "ml" / ".venv"
    if os.name == "nt":
    ...
    if not site_packages or not site_packages.exists():
        raise RuntimeError(
            "ML runtime is not prepared. Expected site-packages at: "
            f"{site_packages}. Prepare agents/runtime/python/ml/.venv first."
        )
    sys.path.insert(0, str(site_packages))


_add_ml_runtime_site_packages()
