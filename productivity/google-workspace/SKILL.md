...
Do not install dependencies inside this skill directory. `setup.py --check-deps`
checks whether the shared runtime is prepared; it does not install anything.
...
| `ModuleNotFoundError` | Run `$GSETUP --check-deps`; if it fails, prepare `agents/runtime/python/base/.venv` (see `runtime/README.md`) |
