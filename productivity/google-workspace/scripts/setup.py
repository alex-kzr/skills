...
Commands:
  ...
  setup.py --check-deps                     # Check shared-runtime dependencies only
...
from _runtime import add_runtime_site_packages
...
def check_deps():
    """Check that Google API packages are available via the shared runtime. Returns True/False."""
    try:
        add_runtime_site_packages()
        import googleapiclient  # noqa: F401
        import google_auth_oauthlib  # noqa: F401
        print("Dependencies available via agents/runtime/python/base/.venv.")
        return True
    except (ImportError, RuntimeError) as e:
        print(f"ERROR: Google API dependencies are not available ({e})")
        print("Prepare the shared runtime first: agents/runtime/python/base/.venv")
        print("See runtime/README.md for setup commands.")
        return False


def _ensure_deps():
    """Check deps are available via the shared runtime, exit on failure."""
    try:
        add_runtime_site_packages()
        import googleapiclient  # noqa: F401
        import google_auth_oauthlib  # noqa: F401
...
def main():
    ...
    group.add_argument("--check-deps", action="store_true", help="Check shared-runtime dependencies")
