...
def validate_input(raw: Any) -> dict:
    ...
    if enabled:
        _reject_unknown(
            graphify,
            {"enabled", "wrapper_source", "wrapper_destination"},
            "graphify",
        )
...
_GRAPHIFYIGNORE = (
    "# Generated by feature-pipeline-project-setup.\n"
    "# Tracked configuration stays; only build output is excluded from indexing.\n"
    "graphify-out/\n"
).encode("utf-8")
...
def render_all(data: dict, project_name: str) -> "dict[str, bytes]":
    """Return the full {relative POSIX path: bytes} map of files to write.

    ``.gitignore`` is handled separately (it is merged, not overwritten).
    """
    out: dict[str, bytes] = {}
    ...
    if data["graphify"]["enabled"]:
        out["tools/graphify/.graphifyignore"] = _GRAPHIFYIGNORE
    ...
    report = {
        ...
        "commands": [
            ...
            for name, summary in (
                ...
                ("path-safety", "all generated paths anchor-relative under tools/"),
