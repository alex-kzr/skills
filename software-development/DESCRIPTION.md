...
| Skill | [feature-pipeline-operator/](feature-pipeline-operator/SKILL.md) | Operate the portable feature pipeline across projects: preflight, execution, recovery, and evidence verification. | Open when running, resuming, monitoring, recovering, or validating a project pipeline run. |
...
| Skill | [pipeline-runtime-recovery/](pipeline-runtime-recovery/SKILL.md) | Recover from a pipeline worker quota or adapter failure without weakening runtime-isolation or verification requirements. | Open when a pipeline worker hits a quota limit or adapter failure. |
