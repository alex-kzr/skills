...
| Skill | [feature-pipeline/](feature-pipeline/SKILL.md) | Develop, diagnose, and understand the portable feature-pipeline core. | Open when changing or investigating the portable core itself; use the operator skill to run a project pipeline. |
| Skill | [feature-pipeline-operator/](feature-pipeline-operator/SKILL.md) | Operate the portable feature pipeline across projects: isolated-worktree inputs, adapter preflight, verifier runtime hardening, execution, recovery, and evidence verification. | Open when running, resuming, monitoring, recovering, or validating a project pipeline run, or preparing an authorized direct workflow. |
| Skill | [pipeline-runtime-fallback/](pipeline-runtime-fallback/SKILL.md) | Apply an authorized direct-runtime fallback while preserving immutable pipeline evidence and independent-verification boundaries. | Open when a pipeline run needs an authorized coding-runtime fallback. |
| Skill | [pipeline-runtime-recovery/](pipeline-runtime-recovery/SKILL.md) | Diagnose worker quota, adapter, and dependency-chain capability failures while preserving immutable runs, isolation, and independent verification. | Open when a pipeline worker hits a quota limit or adapter failure, including a predecessor scheduled because dependency evidence cannot be reused. |
