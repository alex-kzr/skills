...
description: >
  ...
  Also includes Apollo GraphQL best-practice handbook chapters and community
  coding-style rules with clippy lint references. Keywords: naming, formatting,
  clippy, rustfmt, lint, code style, best practice, idiomatic Rust, patterns.
...
The following reference materials from consolidated skills are available under `references/`:

- **Apollo Handbook** (`references/apollo-handbook/`) — 9 chapters from Apollo GraphQL's Rust best-practices handbook covering ownership, borrowing, error handling, performance, testing, and documentation.
- **Coding Guidelines Index** (`references/coding-guidelines-index/`) — Community-sourced Rust coding style rules (naming conventions, formatting, comment standards) with rule IDs.
- **Clippy Lint Reference** (`references/clippy-lints/`) — Detailed clippy lint descriptions organized by category.
