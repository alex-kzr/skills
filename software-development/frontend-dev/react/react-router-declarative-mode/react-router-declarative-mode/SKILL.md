...
> **Part of the React Router v7 family.** This is **Declarative mode** (`<BrowserRouter>` + JSX `<Route>`, no loaders/actions). For the other modes see `react-router-framework-mode` (file-based full-stack, the umbrella hub with a mode-selection table) and `react-router-data-mode` (`createBrowserRouter` + route objects with loaders/actions).
